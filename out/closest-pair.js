...
function closestPair(Px, Py) {
    ...
    if (Px.length < 3) {
        ...
        var d = Math.sqrt(Math.pow(Math.abs(Px[1].x - Px[0].x), 2) +
    ...
    const n = Px.length, leftN = Math.floor(n / 2), rightN = leftN;
    const Xl = Px.slice(0, leftN), Xr = Px.slice(rightN), Xm = Xl[leftN - 1], Yl = [], Yr = [];
    // Separate Py
    for (const py of Py) {
        if (py.x <= Xm.x) {
            Yl.push(py);
    ...
    const closeY = [];
    for (const py of Py) {
        if (Math.abs(py.x - Xm.x) < minDelta)
            closeY.push(py);
    }
    // Find min within delta. 8 steps max
    for (let i = 0; i < closeY.length; i += 1) {
        for (let j = i + 1; j < Math.min(i + 8, closeY.length); j += 1) {
            let d = Math.sqrt(Math.pow(Math.abs(closeY[j].x - closeY[i].x), 2) +
...
//# sourceMappingURL=closest-pair.js.map